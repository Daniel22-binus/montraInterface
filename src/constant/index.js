//color
export const PRIMARY_COLOR = '#697FA3';
export const WHITE = '#fff';
export const SECONDARY_COLOR = '#BACACB';
export const GREEN_COLOR = '#2DB521';
export const BACKGROUND_COLOR = '#9ABCDD';
export const GRAY_COLOR = '#DBD5D5';
export const PROGRESSBAR_COLOR="#6A80A4"
export const SOFT_COLOR="#8EA0BE"

//font
export const PRIMARY_FONT = 'Inter-Regular';
export const BOLD_FONT = 'Inter-Bold';
export const TITLE_FONT = 'MarkoOne-Regular';

export const TITLE_COLOR = '#394267';
export const DETAIL_COLOR = PRIMARY_COLOR;
